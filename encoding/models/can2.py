...
__all__ = ['CAN2', 'get_can2']

class CAN2(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, atrous_rates=(12, 24, 36), norm_layer=nn.BatchNorm2d, **kwargs):
        super(CAN2, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = CAN2Head(2048, nclass, norm_layer, self._up_kwargs,atrous_rates)
        if aux:
...
class CAM_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        return y+se*y
def get_can2(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = CAN2(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
