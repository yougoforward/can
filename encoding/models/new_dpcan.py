...
__all__ = ['new_dpcan', 'get_new_dpcan']

class new_dpcan(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, atrous_rates=(12, 24, 36), decoder=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(new_dpcan, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = new_dpcanHead(2048, nclass, norm_layer, self._up_kwargs,atrous_rates)
        if aux:
...
class new_dpcanHead(nn.Module):
    ...
    def forward(self, x, xl):
        ...
        coarse_max = torch.max(coarse, dim=1, keepdim=True)[0]
        coarse_min = torch.min(coarse, dim=1, keepdim=True)[0]
        coarse = (coarse-coarse_min)/(coarse_max-coarse_min + 1e-10)
        pred_max = torch.max(pred, dim=1, keepdim=True)[0]
        pred_min = torch.min(pred, dim=1, keepdim=True)[0]
        coarse = (pred-pred_min)/(pred_max-pred_min + 1e-10)
...
def get_new_dpcan(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = new_dpcan(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
